  ...
  heroSlides: [
    {
      ...
      backgroundImage: "uploads/placeholder1.svg",
      ctaText: "Join Today",
      ctaLink: "#members",
    ...
    },
    {
      ...
      backgroundImage: "uploads/placeholder2.svg",
      ctaText: "Our Journey",
      ctaLink: "#experiences",
      redirectUrl: "",
      openNewTab: false
  ...
  activities: [
    {
      ...
      image: "uploads/placeholder1.svg",
      status: "upcoming",
      type: "match",
      priority: "high",
    ...
    },
    {
      ...
      image: "uploads/placeholder2.svg",
      status: "upcoming",
      type: "event",
      priority: "high",
  ...
  members: [
    {
      ...
      joinDate: "2023-01-15",
      role: "Team Captain",
      image: "uploads/profile-placeholder.svg"
    },
    {
      id: 2,
      name: "Priya Patel",
...
function deleteGalleryItem(id) {
  if (confirm('Are you sure you want to delete this photo?')) {
    ...
    const itemExists = appData.gallery.some(function(g) { return g.id === id; });
    if (!itemExists) {
    ...
    // Function to handle client-side deletion
    function performClientSideDeletion() {
      ...
      // Reset galleryImages (the local variable used for lightbox)
      // This will be properly repopulated when renderGalleryGrid is called
      galleryImages = [];
    ...
    // First try to delete from the server if API is available
    if (window.API && window.API.gallery && typeof window.API.gallery.delete === 'function') {
      try {
        // Call the API to delete the gallery item with a timeout
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(() => reject(new Error('Request timed out')), 5000);
        });
        
        Promise.race([
          window.API.gallery.delete(id),
          timeoutPromise
        ])
        ...
        .catch(function(error) {
          console.warn('Error with API deletion, falling back to client-side:', error);
          performClientSideDeletion();
        });
      } catch (error) {
        console.warn('Error calling API, falling back to client-side:', error);
        performClientSideDeletion();
      }
    } else {
      // Fallback to client-side only deletion if API is not available
      console.warn('API not available, performing client-side deletion only');
      performClientSideDeletion();
    }
  }
}
